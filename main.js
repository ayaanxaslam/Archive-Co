...
(function () {
  ...
  function cardHTML(p) {
    ...
    if (sold) action = '<button class="enq" disabled>Archived</button>';
      ...
      `<button class="enq enq-add" data-add="${p.id}">Add to cart</button>` +
      `<button class="enq enq-buy" data-buy="${p.id}">Buy now</button>`;
  ...
  /* ---------- home page: featured strip ----------
     The grid is auto-fill, so the column count changes with the viewport.
     We trim the list to a whole number of rows, otherwise the last row sits
     half-empty at some widths and the section looks unfinished.          */
  const feat = document.querySelector("[data-featured]");
  if (feat && typeof PRODUCTS !== "undefined") {
    const max = parseInt(feat.dataset.featured, 10) || 8;
    const pool = PRODUCTS.filter((p) => p.status !== "sold")
      .sort((a, b) => (b.grail === true) - (a.grail === true) || b.price - a.price)
      .slice(0, max);

    const columnCount = () =>
      getComputedStyle(feat).gridTemplateColumns.split(" ").filter(Boolean).length || 1;

    let shown = -1;
    const fill = () => {
      const cols = columnCount();
      /* two full rows once we're wide enough, six pieces when stacked */
      const target = cols >= 3 ? cols * 2 : 6;
      const take = Math.min(target, Math.floor(pool.length / cols) * cols) || pool.length;
      if (take === shown) return;
      shown = take;
      render(feat, pool.slice(0, take));
    };

    fill();
    let t;
    window.addEventListener("resize", () => { clearTimeout(t); t = setTimeout(fill, 160); });
  }
