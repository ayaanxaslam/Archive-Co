...
assets/
  ...
  js/products.js     THE INVENTORY — edit this to change stock
  js/main.js         behaviour + the ordering switch
  favicon.svg
  img/               drop product photography here
```
...
One value, at the top of `assets/js/main.js`:
...
Everything on sale lives in `assets/js/products.js` as one entry per piece:

```js
{ id:"s01", brand:"Nike", name:"Air Max 1 — Patta Waves", cat:"sneakers",
  size:"UK 9", cond:"VNDS", era:"2021", price:340, was:520,
  status:"available", grail:true, img:"assets/img/air-max-1.jpg" },
```
...
**Photography:** drop files into `assets/img/` and point `img` at them. Pieces
without an `img` render a dark monogram tile, so the grid never looks broken
...
- **The contact form has no server.** It composes the message and hands it to
  ...
  replace is marked in `assets/js/main.js`.
